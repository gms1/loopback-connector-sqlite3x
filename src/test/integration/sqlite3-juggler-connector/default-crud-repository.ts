...
import {DefaultCrudRepository, Entity, juggler, model, property, EntityNotFoundError} from '@loopback/repository';
...
describe('sqlite3-juggler-connector: DefaultCrudRepository', () => {
  ...
  it('implements Repository.createAll()', async () => {
    const repo = new DefaultCrudRepository<Note1, number>(Note1, ds);
    const notes = await repo.createAll([{title: 't3', content: 'c3'}, {title: 't4', content: 'c4'}]);
  ...
  it('throws EntityNotFoundError when deleting an unknown id', async () => {
    const repo = new DefaultCrudRepository<Note1, number>(Note1, ds);
    await expect(repo.deleteById(99999))
        .to.be.rejectedWith(
            EntityNotFoundError,
        );
  });
  ...
  it('throws EntityNotFound error when updating an unknown id', async () => {
    const repo = new DefaultCrudRepository<Note1, number>(Note1, ds);
    await expect(repo.updateById(9999, {title: 't4'}))
        .to.be.rejectedWith(
            EntityNotFoundError,
        );
  });
  ...
  it('throws EntityNotFound error when replacing an unknown id', async () => {
    const repo = new DefaultCrudRepository<Note1, number>(Note1, ds);
    await expect(repo.replaceById(9999, {title: 't4'}))
        .to.be.rejectedWith(
            EntityNotFoundError,
        );
  });
