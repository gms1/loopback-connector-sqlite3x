...
describe('DefaultCrudRepository, programmatic ModelDefinition', () => {
  ...
  @model({name: 'note4', settings: {sqlite3x: {tableName: 'NOTE4'}}})
  ...
  beforeEach(async () => {
    ...
    await ds.automigrate('note4');
  });

  afterEach(async () => {
    // tslint:disable-next-line no-shadowed-variable
    const model = ds.createModel<typeof juggler.PersistedModel>('note4');
    model.attachTo(ds);
    await model.deleteAll();
    await connector.dropTable('note4');
    connector.destroyMetaModel('note4');
  });
