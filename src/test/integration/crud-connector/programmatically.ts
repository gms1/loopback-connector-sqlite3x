...
import {Class, CrudRepositoryImpl, Entity, ModelDefinition} from '@loopback/repository';
...
describe('Sqlite3CrudRepository', () => {
  ...
  class Note extends Entity {
    static definition: ModelDefinition = new ModelDefinition({
      ...
      settings: {sqlite3x: {tableName: 'NOTE6'}}
    });
  ...
  beforeEach(async () => {
    ...
    await ds.automigrate('note6');
  });

  afterEach(async () => {
    const model = ds.createModel<typeof juggler.PersistedModel>('note6');
    model.attachTo(ds);
    await model.deleteAll();
    await connector.dropTable('note6');
    ds.destroyMetaModel('note6');
  });
